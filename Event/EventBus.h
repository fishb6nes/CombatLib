...
namespace Combat
{
    ...
    public:
        ...
        AddPreHandler(PreEventHandler<PreEvent> &handler);

        template<class Event> void
        RemoveHandler(EventHandler<Event> &handler);

        template<class PreEvent> void
        RemovePreHandler(PreEventHandler<PreEvent> &handler);

        template<class Event> void
        PublishEvent(const Event &event);

        template<class PreEvent> void
        PublishPreEvent(PreEvent &event);
    };
}
